...
void BinaryOp_1(CuTest *tc)
{
	...
	// root -> rhs
	CuAssertIntEquals(tc, MCC_AST_SINGLE_EXPRESSION_TYPE_LITERAL, expr->rhs->type);

	// root -> rhs -> literal
	CuAssertIntEquals(tc, MCC_AST_LITERAL_TYPE_FLOAT, expr->single_expr->literal->type);
	CuAssertDblEquals(tc, 3.14, expr->single_expr->literal->f_value, EPS);
