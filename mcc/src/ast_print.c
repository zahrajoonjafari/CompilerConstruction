...
static void print_dot_expression_call(struct mcc_ast_expression  *expression, void *data)
{
    ...
    FILE *out = data;
    print_dot_node(out, expression, label);
    for (int i = 0; i < expression->single_expr->arguments->argument_count; i++) {
        print_dot_edge(out, expression, expression->single_expr->arguments->arg_expr[i], "expr");
    }
}
...
static void print_dot_function(struct mcc_ast_function *function, void *data)
{
    ...
    char label[LABEL_SIZE] = { 0 };
    snprintf(label, sizeof(label), "func: %s",mcc_ast_print_data_type(function->function_def->type));
...
static struct mcc_ast_visitor print_dot_visitor(FILE *out)
{
	...
	return (struct mcc_ast_visitor){
	    ...
	    .userdata = out,
			...
			.single_expr_unary_op = print_dot_expression_unary_op,
			.single_expr_identifier = print_dot_expression_identifier,
            .single_expr_call = print_dot_expression_call,
			.single_expr_parenth = print_dot_expression_parenth,
